...
constructor(private servicio:PicoloService, private home:HomeComponent
  ...
  //  this.descripcion= this.getDescripcion(this.retos[this.random].descripcion,this.nombre[this.randomNombre],this.nombre[this.randomSegundoNombre])
    ...
    botellita(){  
      ...
      if(this.contadorBotella==2){
        ...
        getDescripcion(descripcion,nombre1,nombre2):string{
          return descripcion.replace('usuario1',`<span class="nombre-color">${nombre1}</span>` ).replace('usuario2', `<span class="nombre-color">${nombre2}</span>` )
        }
  }
